...
const uint8_t font[] PROGMEM = {
    ...
    class Adafruit_SSD1306 : public Print {
        ...
        Adafruit_SSD1306(uint16_t w, uint16_t h, SPIClass * spi, int8_t dc_pin, int8_t rst_pin, int8_t cs_pin) {
            _init(w, h);
        }

        // Software SPI Constructor (With Dimensions)
        Adafruit_SSD1306(uint16_t w, uint16_t h, int8_t mosi, int8_t sclk, int8_t dc, int8_t rst, int8_t cs) {
            _init(w, h);
        }
        ...
        void drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint8_t size) {
            ...
            if (c >= 176) c++; // Handle extended ASCII mapping if needed (legacy)

            // Adjust for ASCII offset (Font starts at 32)
            if (c < 32) return;
