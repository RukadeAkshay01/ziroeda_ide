...
const SerialMonitor: React.FC<SerialMonitorProps> = ({
  ...
  const handlePopOut = () => {
    ...
    if (newWindow) {
      ...
      newWindow.document.body.style.backgroundColor = '#09090b';
  ...
  const monitorContent = (
    <div className={`flex flex-col bg-dark-900 overflow-hidden ${isPoppedOut ? 'h-screen w-screen' : 'w-[95vw] h-[300px] md:w-[600px] md:h-[350px] rounded-xl border border-dark-700 shadow-2xl'}`}>

      {/* Header */}
      <div
        className={`flex items-center justify-between px-3 py-2 border-b border-dark-700 bg-dark-700 ${!isPoppedOut ? 'cursor-move select-none' : ''}`}
        onMouseDown={!isPoppedOut ? handleMouseDown : undefined}
      >
        <div className="flex items-center gap-2">
          {!isPoppedOut && <GripHorizontal className="w-4 h-4 text-gray-500" />}
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 px-2 py-0.5 bg-dark-900 rounded border border-dark-700">
            <select
              value={baudRate}
              onChange={(e) => setBaudRate(e.target.value)}
              className="bg-transparent text-[9px] font-mono text-gray-400 outline-none border-none cursor-pointer hover:text-white"
          ...
          <button
            ...
            className="p-1 hover:bg-dark-700 text-gray-500 hover:text-white rounded transition-colors"
      ...
      {/* Output */}
      <div className="flex-1 bg-dark-900 p-3 overflow-y-auto font-mono text-xs relative">
        {output.length === 0 ? (
          ...
          <pre className="whitespace-pre-wrap break-all text-brand-50/90 leading-relaxed">{output}</pre>
        )}
      ...
      <div className="bg-dark-800 border-t border-dark-700">
        <div className="px-3 py-1 flex items-center justify-between text-[9px] text-gray-500 border-b border-dark-700/50">
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-gray-300">
            <div className={`w-2.5 h-2.5 border rounded flex items-center justify-center ${autoScroll ? 'bg-brand-500 border-brand-500' : 'border-gray-500'}`}>
        ...
        <div className="p-2">
          <form onSubmit={handleSend} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-dark-900 border border-dark-700 rounded px-2 py-1.5 text-xs text-white font-mono placeholder-gray-500 focus:outline-none focus:border-brand-500/50 disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={!isSimulating || !input}
              className="bg-[#22c55e] hover:bg-[#16a34a] disabled:bg-dark-800 disabled:text-gray-500 text-white px-3 rounded text-[10px] font-bold transition-all flex items-center gap-1"
            >
