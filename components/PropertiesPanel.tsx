...
const PropertiesPanel: React.FC<PropertiesPanelProps> = ({ component, onUpdate, onClose }) => {
  ...
  return (
    <div
      className="fixed z-50 w-72 bg-dark-800 border border-dark-700 rounded-xl shadow-2xl overflow-hidden flex flex-col"
      style={{ left: position.x, top: position.y }}
    >
      {/* Header */}
      <div
        className="h-9 bg-dark-700 border-b border-dark-700 flex items-center justify-between px-3 cursor-move select-none"
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full bg-dark-900 border border-dark-700 rounded px-2 py-1.5 text-xs text-brand-300 focus:border-brand-500/50 outline-none"
        ...
        {relevantAttrs.map((attr) => (
          <div key={attr.key} className="space-y-1">
            <label className="text-[10px] font-bold text-gray-500 uppercase">{attr.label}</label>

            {attr.type === 'select' ? (
              <select
                value={attributes[attr.key] || ''}
                onChange={(e) => setAttributes({ ...attributes, [attr.key]: e.target.value })}
                className="w-full bg-dark-900 border border-dark-700 rounded px-2 py-1.5 text-xs text-gray-300 focus:border-brand-500/50 outline-none"
              >
        ...
        {relevantAttrs.length === 0 && (
          <p className="text-[10px] text-gray-500 italic">No specific editable properties for this component type.</p>
        )}

      </div>

      {/* Footer */}
      <div className="p-3 border-t border-dark-700 bg-dark-700">
        <button
          onClick={handleSave}
          className="w-full bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold py-1.5 rounded flex items-center justify-center gap-2 transition-colors"
        >
