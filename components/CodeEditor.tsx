...
const CodeEditor: React.FC<CodeEditorProps> = ({ code, components, onCodeChange, onClose }) => {
  ...
  const editorContent = (
    <div className={`flex flex-col bg-[#0f1115] overflow-hidden ${isPoppedOut ? 'h-screen w-screen' : 'w-[95vw] h-[70vh] md:w-[800px] md:h-[600px] rounded-xl border border-[#2d3748] shadow-[0_0_50px_rgba(0,0,0,0.5)]'}`}>
        
        {/* Editor Header */}
        <div 
            className={`flex items-center justify-between px-3 md:px-6 py-3 md:py-4 border-b border-[#1e2229] bg-[#13161c] ${!isPoppedOut ? 'cursor-move select-none' : ''}`}
            onMouseDown={!isPoppedOut ? handleMouseDown : undefined}
        >
          <div className="flex items-center gap-3 md:gap-6 flex-1 min-w-0">
            <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
              {!isPoppedOut && <GripHorizontal className="w-5 h-5 text-[#4a5568]" />}
              <div className="bg-cyan-500/10 p-1.5 md:p-2 rounded-lg hidden sm:block">
                <FileCode className="w-4 h-4 md:w-5 md:h-5 text-cyan-400" />
              </div>
              <div className="min-w-0">
                <h3 className="text-xs md:text-sm font-bold text-white uppercase tracking-wider leading-none truncate">Firmware</h3>
                <p className="hidden sm:block text-[10px] text-[#4a5568] font-mono uppercase mt-1">Interactive IDE</p>
              </div>
            </div>

            {/* Board Selector */}
            <div className="h-6 md:h-8 w-px bg-[#2d3748] flex-shrink-0" />
            
            <div className="flex flex-col gap-0.5 md:gap-1 min-w-0 flex-1" onMouseDown={e => e.stopPropagation()}>
              <label className="hidden md:block text-[9px] font-bold text-[#4a5568] uppercase tracking-widest">Target</label>
              <div className="relative group w-full max-w-[140px]">
                <select 
                  value={selectedBoardId}
                  onChange={(e) => setSelectedBoardId(e.target.value)}
                  className="w-full appearance-none bg-[#1a1d24] text-cyan-400 text-[10px] md:text-xs font-mono border border-[#2d3748] rounded px-2 py-1 pr-6 focus:outline-none focus:border-cyan-500/50 cursor-pointer truncate"
                >
                  {boards.length > 0 ? (
                    boards.map(b => (
                ...
                <ChevronDown className="absolute right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 text-[#4a5568] pointer-events-none" />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 md:gap-2 flex-shrink-0 ml-2" onMouseDown={e => e.stopPropagation()}>
            <button
              onClick={handleCopy}
              className={`flex items-center gap-2 px-2 md:px-4 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                copied 
            ...
            <button 
                ...
                className="hidden sm:block p-1.5 hover:bg-[#2d3748] text-[#4a5568] hover:text-white rounded-lg transition-all"
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-8 md:w-12 bg-[#0b0d11] border-r border-[#1e2229] flex flex-col items-center py-6 select-none">
          ...
          <div className="flex-1 relative bg-[#0b0d11]">
            <textarea
              ...
              spellCheck={false}
              className="absolute inset-0 w-full h-full bg-transparent p-4 md:p-6 font-mono text-xs md:text-sm text-cyan-50/90 leading-relaxed resize-none focus:outline-none scrollbar-hide selection:bg-cyan-500/30"
        ...
        {/* Editor Footer */}
        <div className="px-4 md:px-6 py-2 md:py-3 border-t border-[#1e2229] bg-[#13161c] flex items-center justify-between">
            <div className="flex items-center gap-4 md:gap-6 text-[10px] text-[#4a5568] font-mono uppercase tracking-widest">
