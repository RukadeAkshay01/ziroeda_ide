  ...
  const bomItems = useMemo(() => {
    ...
    components.forEach(comp => {
        ...
        if (comp.type.includes('resistor') && comp.attributes.value) {
            description = `${comp.attributes.value}Ω`;
  ...
  const handlePrint = () => {
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          body { font-family: 'Courier New', monospace; padding: 40px; color: #000; max-width: 800px; margin: 0 auto; }
      ...
      <div className="relative bg-[#13161c] w-[95%] md:w-full md:max-w-3xl rounded-xl border border-[#2d3748] shadow-[0_20px_60px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-4 md:px-6 py-5 border-b border-[#1e2229] bg-[#1a1d24]">
          ...
          <div className="flex items-center gap-3">
             <button 
                onClick={handlePrint}
                className="hidden md:flex bg-[#0891b2] hover:bg-[#06b6d4] text-white text-xs font-bold px-4 py-2 rounded-lg transition-all shadow-lg shadow-cyan-900/20 items-center gap-2"
             >
